...
productos.push(new Productos(6,"regadera de metal",1500, url="img/regadera.jpg"));
 ...
 //for of para recorrer mi array y agregar al dom uno por uno
...
//funcion compra usando find y pusheando al carrito
...
// agregando el boton sumar todo con su funcion
let sumarTodoBtn = document.getElementById("btnFinal");
sumarTodoBtn.addEventListener("click", sumarTodo);


 function sumarTodo(n1,n2){
  console.log ("mostrame esto")

    //recorro el carrito con map y me traigo el precio y sumo con reduce
    const sumall = carrito.map(item => item.precio).reduce((prev, curr) => prev + curr, 0);
...
console.log(sumall);
  ...
  let div = document.createElement("div");
  div.className = "Final"
  div.innerHTML =` <h2> Su total es de $ ${sumall}</h2>`
